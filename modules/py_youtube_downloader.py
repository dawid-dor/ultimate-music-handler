...
layout = [[
    ...
    sg.Text('{}'.format(DOWNLOAD_FOLDER))
], [sg.Text("Video URL")], [sg.Input(key='-URL-')], [sg.Text("Video Name")],
          [sg.Input(key='-NAME-')], [sg.Text(size=(40, 1), key='-INFO-')],
...
while True:
    ...
    try:
        ...
        window['-INFO-'].update('Succesfully downloaded a song!',
                                text_color='green')
        # Reset fields
        ...
        window['-INFO-'].update(
            'Could not download song. Please check your URL.',
            text_color='red')
        threading.Timer(3, clear_info).start()
